...
export default function AdminGroupTable({ groups }: { groups: AdminGroupRow[] }) {
  if (groups.length === 0) {
    return <p className="glass-panel p-6 text-sm text-white">Noch keine Gruppen vorhanden.</p>;
  }
        ...
        <tbody>
          {groups.map((group) => (
            <tr key={group._id} className="border-b border-white/5 last:border-0">
              ...
              <td className="py-3 pr-4 text-white">{group.ownerName}</td>
              <td className="py-3 pr-4 text-white">{group.memberCount}</td>
